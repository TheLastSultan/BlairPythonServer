...
# Create a global asynchronous Redis client instance with automatic decoding of responses
    ...
    while retry_count < max_retries:
        try:
            ...
            def shutdown_handler(signum, frame):
                asyncio.create_task(redis_client.close())
            ...
            logger.info("Retrying to connect to Redis... (%s/%s)", retry_count, max_retries)
            await asyncio.sleep(2)

    logger.error("Failed to connect to Redis after retries")
    return None

async def set_redis_data(key: str, value: Any) -> None:
    """
    Sets the given key to the provided value in Redis.
    """
    try:
        result = await redis_client.set(key, value)
        logger.info("Data set in Redis successfully: %s", result)
...
async def set_redis_data_with_ex(key: str, value: Any, ttl: int) -> None:
    """
    Sets the key to the given value with an expiration time (ttl in seconds).
    """
    try:
        result = await redis_client.setex(key, ttl, value)
        logger.info("Data set in Redis with an expiration of %s seconds: %s", ttl, result)
    except Exception as e:
        logger.error("Error setting data with expiration in Redis: %s", e)

async def delete_redis_data(key: str) -> None:
    """
    Deletes the key from Redis.
    """
    try:
        result = await redis_client.delete(key)
        logger.info("Data for key '%s' deleted: %s", key, result)
    except Exception as e:
